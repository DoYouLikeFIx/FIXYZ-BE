...
|---|---|---:|---|---|
| Missing/invalid `X-Internal-Secret` | corebank internal | 401 | AUTH-003 | filter-level reject |
| Status contract validation failure | corebank internal | 400 | BAD_REQUEST | invalid request field |
| Account ownership mismatch | corebank internal | 403 | AUTH-005 | `memberId` not account owner |
| Account not found | corebank internal | 404 | CORE_001 | missing account id |
| Account status unsupported in storage | corebank internal | 422 | VALIDATION-001 | invalid persisted status |
...
- Unknown upstream non-503/504 failures normalize to `INTERNAL_ERROR`.
