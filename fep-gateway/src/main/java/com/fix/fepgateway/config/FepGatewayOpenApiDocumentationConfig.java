...
import com.fix.fepgateway.security.FepGatewayInternalSecretPaths;
...
@Configuration
public class FepGatewayOpenApiDocumentationConfig {
  ...
  @Bean
  OperationCustomizer fepGatewayOpenApiDocumentationDefaults() {
  ...
  @Bean
  OpenApiCustomizer fepGatewayOpenApiSecurityDocumentation() {
    return openApi -> {
      if (openApi.getPaths() == null) {
        return;
      }
      openApi.getPaths().forEach((path, pathItem) -> {
        if (!FepGatewayInternalSecretPaths.requiresInternalSecret(path)) {
          return;
        }
