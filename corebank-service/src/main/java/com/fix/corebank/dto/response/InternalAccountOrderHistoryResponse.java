...
public class InternalAccountOrderHistoryResponse {
  ...
  private InternalAccountOrderHistoryResponse(
      List<Item> content,
      long totalElements,
      int totalPages,
      int number,
  ...
  public static InternalAccountOrderHistoryResponse from(AccountOrderHistoryResult result) {
    List<Item> content = result.getContent().stream()
        .map(Item::from)
        .toList();
    return new InternalAccountOrderHistoryResponse(
        content,
